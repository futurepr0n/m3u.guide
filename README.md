...
- Initializes SQLite database (`instance/app.db`)
- Starts Flask server
...
| `M3UGUIDE_CREDENTIAL_KEY` | Yes | generated into a persistent key file by startup_app.sh | Fernet key used to encrypt provider passwords |
| `M3UGUIDE_CREDENTIAL_KEY_FILE` | No | `.secrets/m3uguide_credential.key` | Persistent fallback key-file location for bare-metal startup |
| `M3UGUIDE_PUBLIC_URL` | Production | request origin | Canonical HTTPS origin used in Jellyfin plugin package URLs |
...
For production, inject `M3UGUIDE_CREDENTIAL_KEY` from your secret manager. For
a single-host installation, `startup_app.sh` safely generates and reuses the
ignored `.secrets/m3uguide_credential.key` file. Back up that file separately
from the database. Losing it makes encrypted provider passwords unrecoverable;
replacing it with a new key does not migrate existing credentials.

### Jellyfin plugin repository

The hosted application publishes the Jellyfin repository at:

```text
https://your-m3u-guide-host/api/jellyfin/plugin-repository/manifest.json
```

Set `M3UGUIDE_PUBLIC_URL=https://your-m3u-guide-host` in production so package
URLs remain correct behind a reverse proxy. Add the manifest URL in Jellyfin at
...
Provider passwords are encrypted at rest with the required
`M3UGUIDE_CREDENTIAL_KEY` deployment secret. See
`docs/CREDENTIAL-KEY-OPERATIONS.md` for generation, backup, recovery, and
rotation procedures. Integration tokens are stored only as SHA-256 digests and
can be inventoried, revoked, or rotated without changing the account password.
Authentication and provider-health endpoints are rate-limited, and API error
responses/log messages redact credential-bearing URLs.
